...
function tableSettings($displayFormResults)
{
    ...
    if (empty($displayFormResults->getBlockByType('accomodation'))) {
        return;
    }
...
function shouldShow($shouldShow, $displayFormResults, $type, $parent)
{
    ...
    // Export buttons
    if (array_intersect_key($bookings->forms->userRoles, $bookings->forms->tableSettings->view_right_roles)) {
...
add_filter('tsjippy-forms-checkbox-options', function ($options, $object) {
    ...
    $bookingSelectors   = $object->getBlockByType('accomodation');
    if (!$bookingSelectors) {
        return $options;
    }
...
{
    if (empty($instance->formData->blockId) || empty($instance->getBlockByType('accomodation'))) {
        return $params;
    }
